...
def _compile_mxfp8_quantize_3d_cutedsl(
    ...
    class Mxfp8Quantize3dKernel:
        ...
        def _store_scale_32x32(
            ...
            scale_u8 = cutlass.Uint8(scale_biased)
            if cutlass.const_expr(BLOCKED_SCALE_OUTPUT):
                # For blocked output, scales_expert is the per-expert
                # 2D replicated scale matrix with logical shape (N,
                # K//32).  Each lane writes the same warp-reduced
                # scale into one row of the 32-row N-block so grouped
                # GEMM can consume the usual replicated blocked
                # layout.
        ...
        def __call__(
            ...
            if cutlass.const_expr(BLOCKED_SCALE_OUTPUT_VALUE):
                ...
                #   replicated across the 32 rows of the N-block to
                #   form a logical (N, K//32) matrix before blocking.
